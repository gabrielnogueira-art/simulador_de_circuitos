...
export interface DetectedElement {
  ...
  params?: Partial<ComponentParams>; // Parâmetros detalhados configurados pelo usuário
...
export class CircuitVision {
  ...
  static generateSchematic(
    ...
    for (const elem of elements) {
      ...
      const defaultRot = isVertical ? 90 : 0;
      const rot = elem.rotation !== undefined ? elem.rotation : defaultRot;

      const comp = createComponent(elem.type, snappedX, snappedY, rot);
      comp.params.label = elem.label;

      // Parsing inicial de valores textuais para os parâmetros numéricos
      this.applyParsedValue(comp, elem.valueText);

      // Aplica parâmetros explícitos configurados pelo usuário se houver (sobrescreve o texto genérico)
      if (elem.params) {
        comp.params = { ...comp.params, ...elem.params };
      }
  ...
  private static applyParsedValue(comp: CircuitComponent, text: string) {
    ...
    const isJ = clean.includes('j');
    const numMatch = clean.match(/[-+]?\d*\.?\d+(?:e[-+]?\d+)?/);
    const num = numMatch ? parseFloat(numMatch[0]) : 0;

    switch (comp.type) {
      case 'RESISTOR':
        if (clean.includes('k')) comp.params.resistance = num * 1e3;
        else if (clean.includes('m') && !clean.includes('mhz') && !clean.includes('mv')) comp.params.resistance = num * 1e6;
        else comp.params.resistance = num || 1000;
        break;

      case 'INDUCTOR':
        if (isJ || clean.includes('ohm') || clean.includes('ω') || clean.includes('Ω')) {
          comp.params.reactance = Math.abs(num) || 6;
          delete comp.params.inductance;
        } else if (clean.includes('mh') || (clean.includes('m') && !clean.includes('meg'))) {
          comp.params.inductance = (num || 10) * 1e-3;
          delete comp.params.reactance;
        } else if (clean.includes('u') || clean.includes('µ')) {
          comp.params.inductance = (num || 10) * 1e-6;
          delete comp.params.reactance;
        } else if (clean.includes('h')) {
          comp.params.inductance = num || 1;
          delete comp.params.reactance;
        } else {
          comp.params.reactance = Math.abs(num) || 6;
        }
      ...
      case 'CAPACITOR':
        if (isJ || clean.includes('ohm') || clean.includes('ω') || clean.includes('Ω')) {
          comp.params.reactance = -Math.abs(num || 16);
          delete comp.params.capacitance;
        } else if (clean.includes('u') || clean.includes('µ')) {
          comp.params.capacitance = (num || 10) * 1e-6;
          delete comp.params.reactance;
        } else if (clean.includes('n')) {
          comp.params.capacitance = (num || 10) * 1e-9;
          delete comp.params.reactance;
        } else if (clean.includes('p')) {
          comp.params.capacitance = (num || 10) * 1e-12;
          delete comp.params.reactance;
        } else if (clean.includes('f')) {
          comp.params.capacitance = num || 1;
          delete comp.params.reactance;
        } else {
          comp.params.reactance = -Math.abs(num || 16);
        }
      ...
      case 'PORT_TERMINAL':
        if (clean.includes('b')) {
          comp.params.portName = 'b';
          comp.params.label = 'b';
        } else {
          comp.params.portName = 'a';
          comp.params.label = 'a';
        }
        break;
