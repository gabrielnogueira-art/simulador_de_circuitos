...
export interface ImpedanceResult {
  ...
  mag: number;
  phaseDeg: number;
  polar: { mag: number; phaseDeg: number };
      ...
      switch (comp.type) {
        ...
        case 'INDUCTOR': {
          if (comp.params.reactance !== undefined) {
            // Reatância direta jX (em Ohms, sempre indutiva +jX)
            const x = Math.abs(comp.params.reactance);
            Z = ComplexMath.create(0, x);
          } else {
        ...
        case 'CAPACITOR': {
          if (comp.params.reactance !== undefined) {
            // Reatância direta -jX (em Ohms, sempre capacitiva -jX)
            const x = comp.params.reactance < 0 ? comp.params.reactance : -comp.params.reactance;
            Z = ComplexMath.create(0, x);
          } else {
